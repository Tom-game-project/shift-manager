...
mod calendar_repo_tests {
    ...
    use shift_manager_tauri_lib::{
        domain::shift_calendar_model::WeekStatus,
        // RuleRepository をインポート
        infrastructure::{calendar_repo::CalendarRepository, rule_repo::RuleRepository},
    };
    ...
    async fn test_fetch_rules_by_ids() {
        ...
        let plan_id = rule_repo
            .create_plan("Test Plan")
            .await
            .expect("Failed to create plan");
        let group_id = rule_repo
            .add_staff_group(plan_id, "Group A")
            .await
            .expect("Failed to create group");

        // Rule A
        let rule_a_id = rule_repo
            .add_weekly_rule(plan_id, "Rule A")
            .await
            .expect("Failed to create rule");
        rule_repo
            .add_rule_assignment(rule_a_id, 0, 0, group_id, 0)
            .await
            .expect("Failed to assign"); // Mon
        rule_repo
            .add_rule_assignment(rule_a_id, 1, 0, group_id, 0)
            .await
            .expect("Failed to assign"); // Tue

        // Rule B
        let rule_b_id = rule_repo
            .add_weekly_rule(plan_id, "Rule B")
            .await
            .expect("Failed to create rule");
        rule_repo
            .add_rule_assignment(rule_b_id, 5, 0, group_id, 0)
            .await
            .expect("Failed to assign"); // Sat

        // Rule C
        let rule_c_id = rule_repo
            .add_weekly_rule(plan_id, "Rule C")
            .await
            .expect("Failed to create rule");
        rule_repo
            .add_rule_assignment(rule_c_id, 6, 0, group_id, 0)
            .await
            .expect("Failed to assign");

        // [Act] Rule A と B だけを取得
        let results = cal_repo
            .fetch_rules_by_ids(&[rule_a_id, rule_b_id])
            .await
        ...
        let res_a = results
            .iter()
            .find(|r| r.rule.id == rule_a_id)
            .expect("Rule A missing");
        assert_eq!(res_a.rule.name, "Rule A");
        assert_eq!(res_a.assignments.len(), 2);

        let res_b = results
            .iter()
            .find(|r| r.rule.id == rule_b_id)
            .expect("Rule B missing");
    ...
    async fn test_save_timeline_debug_print_timeline() {
        ...
        let plan_id = rule_repo
            ...
            .await
            .expect("Failed plan");
        let rule_a = rule_repo
            .add_weekly_rule(plan_id, "Rule A")
            .await
            .expect("Failed rule A");
        let rule_b = rule_repo
            .add_weekly_rule(plan_id, "Rule B")
            .await
            .expect("Failed rule B");

        println!("try to append create_calendar1");
        cal_repo.create_calendar(plan_id, 2920, 100).await.unwrap();

        println!("try to append create_calendar2");
        cal_repo
            .try_to_append_timeline(
                plan_id,
                2920,                                   // ここから生成を開始したい絶対週
                vec![Some(rule_a), Some(rule_b), None], // 配列の所有権をそのまま渡す（イテレータに変換される）
            )
            .await
            .unwrap();
